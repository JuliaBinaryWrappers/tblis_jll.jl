...
if VERSION < v"1.3.0-rc4"
    ...
    error("Unable to import tblis_jll on Julia versions older than 1.3!")
end
    ...
    function dev_jll()
        ...
        mktempdir() do temp_env
            Pkg.activate(temp_env) do
                Pkg.develop("tblis_jll")
            end
        end
        # Create the override directory
        override_dir = joinpath(Pkg.devdir(), "tblis_jll", "override")
        # Copy the current artifact contents into that directory
        if !isdir(override_dir)
            cp(artifact"tblis", override_dir)
        end
        # Force recompilation of that package, just in case it wasn't dev'ed before
        touch(joinpath(Pkg.devdir(), "tblis_jll", "src", "tblis_jll.jl"))
        @info("tblis_ll dev'ed out to /depot/dev/tblis_jll with pre-populated override directory")
    end
end
...
artifacts = Pkg.Artifacts.load_artifacts_toml(artifacts_toml; pkg_uuid=UUID("9c7f617c-f299-5d18-afb6-044c7798b3d0"))
platforms = [Pkg.Artifacts.unpack_platform(e, "tblis", artifacts_toml) for e in artifacts["tblis"]]
...
if best_platform === nothing
    @debug("Unable to load tblis; unsupported platform $(triplet(platform_key_abi()))")
else
